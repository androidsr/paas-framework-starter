...
public class MongoHelper {
    ...
    public Object findLike(String param, String key, String collName, String sortField, Sort.Direction direction, Class<?> cls) {
        ...
        return mongoTemplate.find(query, cls, collName);
    }
    ...
    public PageResult findSortPage(Class<?> clazz, String collName, Map<String, Object> map, int pageNo, int pageSize, Sort.Direction direction, String sortField) {
        ...
        if (criteria == null) {
            count = (int) mongoTemplate.count(new Query(), clazz, collName);
        } else {
            count = (int) mongoTemplate.count(new Query(criteria), clazz, collName);
        }
        ...
        PageResult dataModel = new PageResult();
        dataModel.setCurrent(pageNo);
        dataModel.setSize(pageSize);
        dataModel.setTotal(count);
        dataModel.setTotal(pages);
        dataModel.setRows(list);
        return dataModel;
    }

    private void saveData(String collName, Object entity) {
        if (PaasUtils.isEmpty(collName)) {
